...
class _UserTradesPageState extends ConsumerState<UserTradesPage> {
  ...
  Future<void> _loadInitialTrades() async {
    ...
    try {
      final tradesAsync = await ref.read(tradesProvider.future);
      final userTrades = tradesAsync.where((trade) => trade.userId == widget.userId).toList();
      setState(() {
        _trades = userTrades.take(_pageSize).toList();
        _hasMore = userTrades.length > _pageSize;
        _page = 1;
        _isLoading = false;
      });
    } catch (e) {
  ...
  Future<void> _loadMoreTrades() async {
    ...
    try {
      final tradesAsync = await ref.read(tradesProvider.future);
      final userTrades = tradesAsync.where((trade) => trade.userId == widget.userId).toList();
      final nextBatch = userTrades.skip(_page * _pageSize).take(_pageSize).toList();
